...
$data = file_get_contents('https://api.dxprog.com/?method=content.getContent&contentType=comic&type=json&max=1000');
...
foreach ($data->body->content as $post) {
  ...
  $body = '[![](' . $comicUrl . ')](' . $comicUrl . ')' . PHP_EOL . PHP_EOL . $body;

  $out .= $body . PHP_EOL;
  file_put_contents('./comics/' . $post->perma . '.md', $out);
}