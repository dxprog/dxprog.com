...
$data = file_get_contents('https://api.dxprog.com/?method=content.getContent&contentType=art&type=json&max=1000');
...
foreach ($data->body->content as $post) {
  $imageUrl = $post->meta->file;
  if (strpos($imageUrl, 'http') !== 0) {
    $imageUrl = 'https://dxprog.com' . $imageUrl;
  }
  $out = '---' . PHP_EOL;
  $out .= 'title: "' . str_replace('"', '\\"', $post->title) . '"' . PHP_EOL;
  $out .= 'slug: ' . $post->perma . PHP_EOL;
  $out .= 'date: ' . date('c', (int) $post->date) . PHP_EOL;
  $out .= 'type: art' . PHP_EOL;
  $out .= 'image: ' . $imageUrl . PHP_EOL;
  $out .= 'width: ' . $post->meta->width . PHP_EOL;
  $out .= 'height: ' . $post->meta->height . PHP_EOL;
  ...
  $body = '[![](' . $imageUrl . ')](' . $imageUrl . ')' . PHP_EOL . PHP_EOL . $body;

  $out .= $body . PHP_EOL;
  file_put_contents('./art/' . $post->perma . '.md', $out);
}